...
export default function SocialScreen() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    try { const [nextSocial, nextGuilds] = await Promise.all([classeId > 0 ? carregarSocial(classeId) : carregarSocial(), classeId > 0 ? carregarGuildas(classeId) : Promise.resolve([])]); setSocial(nextSocial); setGuilds(nextGuilds); }
