...
export type SocialPerson = { alunoId: string; nome: string; apelido: string | null; fotoUrl: string | null; perfilAtivo: string | null; status: SocialStatus; relationshipId: string | null };
...
export function normalizeSocialRow(row: Record<string, unknown>): SocialPerson {
  return { alunoId: String(row.aluno_id), nome: String(row.nome ?? "Colega"), apelido: (row.apelido as string | null) ?? null, fotoUrl: (row.foto_url as string | null) ?? null, perfilAtivo: (row.perfil_ativo as string | null) ?? null, status: String(row.status ?? "candidate") as SocialStatus, relationshipId: (row.relationship_id as string | null) ?? null };
}
