...
export type SocialPeopleSection = "friends" | "invites" | "discover" | "blocked";
...
export function peopleForSocialSection(snapshot: SocialSnapshot, section: SocialPeopleSection): SocialPerson[] {
  if (section === "friends") return snapshot.friends;
  if (section === "invites") return [...snapshot.incoming, ...snapshot.outgoing];
  if (section === "discover") return snapshot.candidates;
