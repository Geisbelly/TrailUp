...
import { validateBagDraft, type BagDraft, type BagItem, type BagItemType } from "@/services/bag/bagModel";
...
export function BagEditorModal({ visible, initialItem, context, accent, surface, muted, onClose, onSave }: Props) {
  const [type, setType] = useState<BagItemType>(initialItem?.type ?? "anotacao");
  ...
  useEffect(() => {
    if (!visible) return;
    setType(initialItem?.type ?? "anotacao"); setTitle(initialItem?.title ?? ""); setContent(initialItem?.content ?? ""); setFront(initialItem?.front ?? ""); setBack(initialItem?.back ?? ""); setError(null);
  }, [initialItem, visible]);
