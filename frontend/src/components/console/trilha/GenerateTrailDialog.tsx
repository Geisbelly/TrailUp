    ...
    <Dialog open={open} onOpenChange={close}>
      <DialogContent className="max-w-2xl bg-[#0F172A] border-slate-800 text-slate-200 max-h-[90vh] flex flex-col overflow-hidden">
        <div className="flex items-center gap-3 pb-2">
          ...
          <div><DialogTitle className="text-lg font-bold text-white">Gerar Trilha com IA</DialogTitle><DialogDescription className="text-slate-400 text-xs mt-0.5">Arquivos são enviados ao bucket somente ao confirmar a criação.</DialogDescription></div>
        </div>
        <ScrollArea className="flex-1 overflow-y-auto pr-1">
          {(step === "form" || step === "confirm-delete") && <div className="space-y-5 py-2">
            <div><Label className="text-slate-400 text-[11px] font-bold uppercase tracking-wider mb-1 block">Descrição *</Label><Textarea value={description} onChange={(e) => setDescription(e.target.value)} className="bg-[#111827] border-slate-700 text-slate-100 placeholder:text-slate-600 focus:border-violet-500 min-h-[72px] resize-none" /></div>
            <div><Label className="text-slate-400 text-[11px] font-bold uppercase tracking-wider mb-1 block">Qtd. tópicos</Label><Input type="number" min={2} max={20} value={numTopics} onChange={(e) => setNumTopics(Math.min(20, Math.max(2, Number(e.target.value) || 2)))} className="bg-[#111827] border-slate-700 text-slate-100 focus:border-violet-500 w-24" /></div>
            <div><Label className="text-slate-400 text-[11px] font-bold uppercase tracking-wider mb-1 block">Tópicos (opcional)</Label><Textarea value={topicNamesRaw} onChange={(e) => setTopicNamesRaw(e.target.value)} className="bg-[#111827] border-slate-700 text-slate-100 min-h-[80px] resize-none font-mono text-sm" /></div>
