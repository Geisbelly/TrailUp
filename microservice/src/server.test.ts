...
describe("POST /api/personalizar concorrencia", () => {
  ...
  it("mantem heartbeat no job que ainda espera na fila do gate", { timeout: 10_000 }, async () => {
    // `recoverStaleJobs` marca como órfã qualquer linha `processando_midias`
    // com `updated_at` mais velho que 150s. O heartbeat do pipeline nascia
    // DEPOIS do gate, então o job em espera ficava sem ninguém tocando
    // `updated_at` -- congelado no instante em que a API criou a linha -- e
    // era reapado antes da vez dele. Medido no tópico 128: seeker e
    // socializer, "idade 534s", zero processamento.
    ...
    const { base, close } = await startTestServer({
      maxConcurrentPersonalizacaoJobs: 1,
      startJobHeartbeat: (personalizacaoId: number) => {
        iniciados.push(personalizacaoId);
        return () => {
          parados.push(personalizacaoId);
        };
      },
      personalizacaoJobRunner: () => new Promise<void>((resolve) => {
    ...
    let requests: Promise<Response>[] = [];
    try {
      requests = [post(1), post(2)];
      await new Promise((resolve) => setTimeout(resolve, 80));

      // O gate deixa só um rodar, mas os DOIS precisam ter heartbeat: é
      // exatamente o segundo que morria.
      assert.equal(releases.length, 1);
      assert.deepEqual([...iniciados].sort(), [1, 2]);
      ...
      // E os dois param no fim -- senão o setInterval vaza por job.
      assert.deepEqual([...parados].sort(), [1, 2]);
    } finally {
      while (releases.length) releases.shift()!();
      await Promise.allSettled(requests);
      await close();
