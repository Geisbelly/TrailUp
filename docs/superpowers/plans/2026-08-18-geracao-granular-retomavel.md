...
1. **Task 12b** (mencionado na nota do Task 12, Step 3): fechar `persistir_parte_fn` real e o merge final em `conteudo_personalizado.materiais` do lado Python, equivalente a `mergePersonalizacaoMateriais` do microservice.
...
4. Rodar a suíte `microservice` completa (`cd microservice && npm run lint && npm test`) e a suíte `api` completa (`cd api && python -m pytest`) uma última vez antes de abrir PR — nenhum task individual acima cobre uma rodada final de ponta a ponta dos dois repositórios juntos.
