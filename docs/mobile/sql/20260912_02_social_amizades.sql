...
RETURNS TABLE(aluno_a_id uuid, aluno_b_id uuid) LANGUAGE sql IMMUTABLE AS $fn$
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, pg_temp AS $fn$
  SELECT p_one <> p_two AND EXISTS (
    SELECT 1 FROM classe_aluno a JOIN classe_aluno b ON b.classe_id = a.classe_id
...
BEGIN
  UPDATE public.social_relacionamentos SET status='accepted', responded_at=now(), updated_at=now()
   WHERE id=p_relationship_id AND (aluno_a_id=auth.uid() OR aluno_b_id=auth.uid()) AND status='pending' AND solicitante_id <> auth.uid();
...
BEGIN
  ...
  ON CONFLICT (aluno_a_id,aluno_b_id) DO UPDATE SET status='blocked',blocked_by_id=v_me,updated_at=now(),responded_at=now() RETURNING id INTO v_id;
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, pg_temp AS $fn$
  SELECT CASE WHEN r.aluno_a_id=auth.uid() THEN r.aluno_b_id ELSE r.aluno_a_id END, a.nome,a.apelido,a.foto_url,a.perfil_ativo,
    ...
    FROM social_relacionamentos r JOIN alunos a ON a.id = CASE WHEN r.aluno_a_id=auth.uid() THEN r.aluno_b_id ELSE r.aluno_a_id END
   WHERE (r.aluno_a_id=auth.uid() OR r.aluno_b_id=auth.uid())
  UNION ALL
  SELECT a.id,a.nome,a.apelido,a.foto_url,a.perfil_ativo,'candidate',NULL FROM alunos a
   WHERE a.id IN (SELECT public.app_colegas_de_turma()) AND a.id <> auth.uid()
     AND NOT EXISTS (SELECT 1 FROM social_relacionamentos r WHERE r.aluno_a_id=LEAST(auth.uid(),a.id) AND r.aluno_b_id=GREATEST(auth.uid(),a.id));
$fn$;
