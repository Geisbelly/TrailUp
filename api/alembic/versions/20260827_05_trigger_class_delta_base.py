"""Trigger de class-delta gera base por perfil, sem depender de classe_aluno

A 20260827_03 pos o enfileiramento de class_delta_sync no banco, mas montava os
targets elegendo um aluno representante por perfil a partir de `classe_aluno` e
filtrava `WHERE r.aluno_id IS NOT NULL`. Em turma vazia isso criava o job e zero
target -- o mesmo 0/0 do caminho manual, agora pelo automatico.

Com a 20260827_04, `aluno_id` nulo em personalizacao_job_targets significa base
da classe. O trigger passa a emitir direto os 7 perfis por (topico, conteudo),
sem dono. As CTEs `alunos`, `dominante` e `representante` somem; `escopo` fica
identico.

Duas sutilezas que vieram junto:

- O `ON CONFLICT` muda de indice: agora e `uq_job_target_base`, e o predicado
  parcial (`media_kind IS NULL AND aluno_id IS NULL`) precisa ser repetido,
  senao o Postgres levanta "no unique or exclusion constraint matching".

- O `target_profile_map` passa a ser `{}` sempre. A chave dele e
  (aluno, topico, conteudo) e o perfil e o VALOR: sem aluno, os 7 perfis
  colapsariam numa chave so e 6 se perderiam em silencio. Para a base ele e
  dispensavel, porque `brainhex_profile_key` vem na propria linha do target --
  que e a fonte primaria em _process_media_render_target.

Os triggers criados pela 20260827_03 continuam valendo: so o corpo da funcao
muda.
...
FN_ENQUEUE = """CREATE OR REPLACE FUNCTION public.fn_enqueue_class_delta_job()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- 4. Targets: 7 perfis BrainHex x (topico, conteudo), SEM dono. A base e
  --    material de (classe x topico x conteudo x perfil) e existe com ou sem
  --    aluno matriculado -- antes cada perfil era pendurado num representante
  --    tirado de classe_aluno, e turma vazia nao gerava nada (job 0/0).
  ...
  FROM escopo e
  CROSS JOIN perfis p
  -- ON CONFLICT sobre indice PARCIAL exige repetir o predicado (uq_job_target_base).
  ON CONFLICT (job_id, topico_id, conteudo_id, brainhex_profile_key)
    WHERE media_kind IS NULL AND aluno_id IS NULL
  DO NOTHING;

  -- 5. Contadores saem do que FOI persistido, nunca do conjunto pre-insercao:
  --    numa fusao o job ja tem targets de eventos anteriores. O
  --    target_profile_map fica vazio de proposito -- a chave dele e (aluno,
  --    topico, conteudo) e o perfil e o VALOR, entao sem aluno os 7 perfis
  --    colapsariam numa chave so. Para a base ele e dispensavel:
  --    brainhex_profile_key vem na propria linha do target.
...
# Corpo da 20260827_03, restaurado no downgrade: volta a eleger representante
# a partir de classe_aluno.
FN_ENQUEUE_20260827_03 = """CREATE OR REPLACE FUNCTION public.fn_enqueue_class_delta_job()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
def downgrade() -> None:
    op.execute(FN_ENQUEUE_20260827_03)
