"""Use the strongest stored BrainHex profile in public student profiles."""
    ...
    RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, pg_temp AS $$
      WITH turma_compartilhada AS (
        SELECT ca.classe_id
          ...
          JOIN public.classe_aluno eu ON eu.classe_id=ca.classe_id AND eu.aluno_id=auth.uid()
        ...
        SELECT p.nome, p.descricao
         ...
         ORDER BY
           CASE WHEN lower(btrim(p.nome)) = lower(btrim((SELECT a.perfil_ativo FROM public.alunos a WHERE a.id=p_aluno_id))) THEN 0 ELSE 1 END,
      ...
      SELECT jsonb_build_object(
        ...
        'perfil_ativo',COALESCE(NULLIF(btrim(a.perfil_ativo),''),p.nome),'descricao',a.descricao,'perfil_descricao',p.descricao,
        'guilda', (SELECT jsonb_build_object('id',g.id,'nome',g.nome,'emblema',g.emblema,'membros',g.limite_membros)
          FROM public.guilda_membros gm JOIN public.guildas g ON g.id=gm.guilda_id WHERE gm.aluno_id=a.id AND gm.classe_id=tc.classe_id AND gm.left_at IS NULL AND g.ativa LIMIT 1),
        'conquistas', COALESCE((SELECT jsonb_agg(jsonb_build_object('id',c.id,'nome',c.nome,'descricao',c.descricao,'icone_url',c.icone_url,'categoria',c.categoria,'pontos_recompensa',c.pontos_recompensa,'data_conquista',ca.data_conquista) ORDER BY ca.data_conquista DESC)
          FROM public.conquistas_aluno ca JOIN public.conquistas c ON c.id=ca.conquista_id WHERE ca.aluno_id=a.id AND ca.concluida IS TRUE AND (c.escopo IS NULL OR c.escopo='comum' OR c.perfil_alvo=COALESCE(NULLIF(btrim(a.perfil_ativo),''),p.nome))), '[]'::jsonb),
...
def downgrade() -> None:
    raise RuntimeError("Downgrade manual: manter fallback de perfil por afinidade")
