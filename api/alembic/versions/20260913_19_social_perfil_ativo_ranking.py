"""Use each student's active profile in ranking/social rows."""

from alembic import op


revision = "20260913_19"
down_revision = "20260913_18"
branch_labels = None
depends_on = None
    ...
    op.execute(
        ...
        AS $fn$
          ...
          SELECT CASE WHEN r.aluno_a_id = auth.uid() THEN r.aluno_b_id ELSE r.aluno_a_id END,
            ...
            JOIN perfil_publico pp ON pp.id = a.id
...
def downgrade() -> None:
    raise RuntimeError("Downgrade manual: manter perfil ativo no ranking")
