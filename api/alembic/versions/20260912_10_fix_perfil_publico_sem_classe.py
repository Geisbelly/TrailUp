"""Resolve the shared class when the mobile screen has not hydrated its class yet."""
...
def upgrade() -> None:
    ...
    RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, pg_temp AS $$
      ...
      LEFT JOIN public.aluno_perfil ap ON ap.aluno_id=a.id AND a.perfil_ativo=(SELECT nome FROM public.perfil WHERE id=ap.perfil_id LIMIT 1)
...
def downgrade() -> None:
    raise RuntimeError("Downgrade manual: manter resolução segura da turma compartilhada")
